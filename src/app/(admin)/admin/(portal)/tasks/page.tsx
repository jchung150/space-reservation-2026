      ...
      <div style={{ background: '#fff', borderRadius: 12, border: `1px solid ${C.border}`, padding: '14px 16px', marginBottom: 16, display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 200px', minWidth: 200 }}>
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ width: '100%', padding: '8px 12px 8px 34px', borderRadius: 8, border: `1.5px solid ${C.border}`, fontSize: 13, color: C.textPri, outline: 'none', fontFamily: 'inherit', background: C.pageBg, transition: '150ms ease' }}
          />
        </div>
        ...
        <FilterSelect value={statFilter} onChange={v => { setStatFilter(v); setPage(1); }} options={['전체','미완료','검토 대기','완료','재작업']} minWidth={150} />
        {selected.size > 0 && (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: C.pageBg, borderBottom: `1px solid ${C.border}` }}>
                ...
                {['업무명','배정 직원','직군','우선순위','마감일시','상태','반복','액션'].map(h => (
              ...
              {pagedTasks.length === 0 ? (
                ...
                return (
                  <tr key={task.id}
                    ...
                    style={{ borderTop: `1px solid ${C.border}`, background: isSel ? C.primaryBg : isHov ? C.pageBg : 'transparent', transition: '120ms' }}>
                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      <div onClick={() => toggleOne(task.id)}
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {isDel ? (
                        ...
                        <div style={{ display: 'flex', gap: 4 }}>
                          {/* 상세보기 */}
                          <Link href={`/admin/tasks/${task.id}`} title="상세보기" className="admin-btn-icon admin-btn-edit">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                          </Link>
                          {/* 수정 */}
                          <Link href={`/admin/tasks/${task.id}/edit`} title="수정" className="admin-btn-icon admin-btn-edit">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                          </Link>
                          {/* 삭제 */}
                          <button type="button" title="삭제" className="admin-btn-icon admin-btn-delete" onClick={() => setDeletingId(task.id)}>
