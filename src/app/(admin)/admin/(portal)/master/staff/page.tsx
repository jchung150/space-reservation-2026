...
/* ── 통합 행 (직원 + 관리자) ─────────────────────────────────── */
type UnifiedRow =
  | { kind: 'staff'; data: StaffRow }
  | { kind: 'admin'; data: AdminRow };
...
function DeleteWarningDialog({
  ...
  return (
    <div onClick={onClose}
      style={{ position: 'fixed', inset: 0, background: 'oklch(0% 0 0 / 50%)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 24 }}>
      <div onClick={e => e.stopPropagation()}
        style={{ width: '100%', maxWidth: 360, background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 20px 60px oklch(0% 0 0 / 30%)' }}>
        <div style={{ padding: '28px 24px 20px', textAlign: 'center' }}>
          ...
          <div style={{ background: 'oklch(95% 0.04 25)', border: '1px solid oklch(88% 0.06 25)', borderRadius: 10, padding: '12px 14px', marginBottom: 12, textAlign: 'left' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.danger, marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {error && (
            <div style={{ marginTop: 12, padding: '8px 12px', background: 'oklch(95% 0.04 25)', borderRadius: 8, fontSize: 12, color: C.danger, fontWeight: 600 }}>
              {error}
            </div>
          )}
        </div>
        <div style={{ display: 'flex', borderTop: `1px solid ${C.border}` }}>
          <button type="button" onClick={onClose} disabled={loading}
            style={{ flex: 1, padding: '15px', border: 'none', background: 'transparent', fontSize: 15, fontWeight: 600, color: C.textSec, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', borderRight: `1px solid ${C.border}` }}>
            취소
          </button>
          <button type="button" onClick={onConfirm} disabled={loading}
            style={{ flex: 1, padding: '15px', border: 'none', background: 'transparent', fontSize: 15, fontWeight: 700, color: C.danger, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: loading ? 0.6 : 1 }}>
            {loading ? '삭제 중...' : '삭제하기'}
          </button>
        </div>
      </div>
...
function AdminEditModal({ target, onClose, onSave }: {
  ...
  function inputStyle(fieldName: string): React.CSSProperties {
    ...
    return { width: '100%', padding: '10px 12px', borderRadius: 8, border: `1.5px solid ${isFoc ? C.primary : C.border}`, background: isFoc ? '#fff' : C.pageBg, fontSize: 14, color: C.textPri, outline: 'none', fontFamily: 'inherit', transition: '150ms ease' };
  }
  ...
  return (
    <div onClick={onClose}
      style={{ position: 'fixed', inset: 0, background: 'oklch(0% 0 0 / 45%)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }}>
      <div onClick={e => e.stopPropagation()}
        style={{ width: 480, maxHeight: '90vh', overflowY: 'auto', background: '#fff', borderRadius: 14, boxShadow: '0 20px 60px oklch(0% 0 0 / 25%)' }}>
        <div style={{ padding: '18px 22px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h2 style={{ fontSize: 17, fontWeight: 700, color: C.textPri }}>관리자 정보 수정</h2>
            <p style={{ fontSize: 12, color: C.textMuted, marginTop: 2 }}>{target.name} 계정 정보를 수정합니다</p>
          </div>
          <button type="button" onClick={onClose}
            style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: 'transparent', cursor: 'pointer', color: C.textMuted, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ padding: 22 }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <input value={name} onChange={e => { setName(e.target.value); setErr(''); }} onFocus={() => setFocused('name')} onBlur={() => setFocused(null)} placeholder="이름" style={inputStyle('name')} />
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: C.textPri, marginBottom: 6, display: 'block' }}>아이디 변경 (선택)</label>
            <input value={loginId} onChange={e => { setLoginId(e.target.value); setErr(''); }} onFocus={() => setFocused('loginId')} onBlur={() => setFocused(null)} placeholder="변경할 아이디 (비워두면 유지)" style={inputStyle('loginId')} autoComplete="off" />
          </div>
          ...
          {err && <p style={{ fontSize: 12, color: C.danger, fontWeight: 600, marginTop: 12 }}>{err}</p>}
        </div>
        ...
        <div style={{ padding: '14px 22px', borderTop: `1px solid ${C.border}`, background: C.pageBg, borderRadius: '0 0 14px 14px', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" onClick={onClose}
            style={{ padding: '10px 22px', borderRadius: 8, border: `1.5px solid ${C.border}`, background: '#fff', fontSize: 13, fontWeight: 600, color: C.textSec, cursor: 'pointer', fontFamily: 'inherit' }}>취소</button>
          <button type="button" onClick={handleSave} disabled={saving}
            style={{ padding: '10px 26px', borderRadius: 8, border: 'none', background: saving ? 'oklch(75% 0.08 195)' : C.primary, color: '#fff', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit', boxShadow: '0 2px 8px oklch(55% 0.14 195 / 30%)' }}>
            {saving ? '저장 중...' : '수정 저장'}
          </button>
        </div>
      </div>
...
export default function MasterStaffPage() {
  ...
  const { data: adminList = [], isLoading: adminLoading } = useQuery<AdminRow[]>({
    queryKey: ['admin-list'],
  ...
  const rows = useMemo<UnifiedRow[]>(() => {
    return [
      ...adminList.map(a => ({ kind: 'admin' as const, data: a })),
      ...staffList.map(s => ({ kind: 'staff' as const, data: s })),
    ];
  }, [staffList, adminList]);

  const isLoading = staffLoading || adminLoading;
    ...
    <div>
      <div style={{ background: '#fff', borderRadius: 12, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
        {isLoading ? (
          ...
          <div style={{ padding: '48px 16px', textAlign: 'center', color: C.textMuted, fontSize: 14 }}>등록된 직원이 없습니다</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: C.pageBg, borderBottom: `1px solid ${C.border}` }}>
                {['이름', '구분', '직군', '연락처', '계정 상태', '액션'].map(h => (
            ...
            <tbody>
              {rows.map(row => {
                if (row.kind === 'admin') {
                  const a = row.data;
                  return (
                    <tr key={`admin-${a.id}`} style={{ borderTop: `1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: a.role === 'super' ? C.warning : C.primary, background: a.role === 'super' ? C.warningBg : C.primaryBg, borderRadius: 6, padding: '3px 8px' }}>
                          {a.role === 'super' ? '슈퍼 관리자' : '관리자'}
                        </span>
                      </td>
                      <td style={{ padding: '12px 16px', fontSize: 13, color: C.textMuted }}>—</td>
                ...
                const s = row.data;
                return (
                  <tr key={`staff-${s.id}`} style={{ borderTop: `1px solid ${C.border}`, background: s.isActive ? 'transparent' : C.inactiveBg, opacity: s.isActive ? 1 : 0.65 }}
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ fontSize: 11, fontWeight: 600, color: C.textSec, background: C.pageBg, border: `1px solid ${C.border}`, borderRadius: 6, padding: '3px 8px' }}>직원</span>
                    </td>
                    <td style={{ padding: '12px 16px' }}>
